...
from core.testing_examples.models import Roster
from core.testing_examples.services import (
    ROSTER_CREATE_DIFFERENT_SCHOOLS,
    roster_create,
)
from core.testing_examples.tests.factories import (
    SchoolCourseFactory,
    StudentFactory,
)
...
class RosterCreateTests(TestCase):
    def test_service_raises_error_if_different_schools(self):
        ...
        with self.assertRaisesMessage(
            ValidationError, ROSTER_CREATE_DIFFERENT_SCHOOLS.format(student=student, school_course=school_course)
        ):
            roster_create(student=student, school_course=school_course)

        self.assertEqual(Roster.objects.count(), 0)

    @patch("core.testing_examples.services.rosters.roster_validate_period")
    ...
    def test_service_doesn_not_school_course_period_if_dates_are_passed(self, roster_validate_period_mock):
        ...
        roster = roster_create(student=student, school_course=school_course, start_date=start_date, end_date=end_date)
